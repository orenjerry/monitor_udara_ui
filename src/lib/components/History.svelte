<script lang="ts">
    ...
    let historyMap = $state<Record<string, DayData>>({});
    let hasHistoryData = $derived(Object.keys(historyMap).length > 0);

    // Fungsi utilitas untuk memformat YYYY-MM-DD dari waktu lokal perangkat
    function formatLocalDateString(dt: Date): string {
        ...
        getHistory(DEMO_MAC)
            .then((rows: any[]) => {
                ...
                for (const r of rows) {
                    ...
                    let safeDateString: string | undefined;
                    if (rawDate) {
                        if (rawDate.includes(' ')) {
                            // Fallback jika masih ada data lama di DB yang pakai spasi & tanpa timezone
                            safeDateString = rawDate.split('.')[0].replace(' ', 'T') + '+07:00';
                        } else {
                            // Format baru dari API yang sudah memakai isoformat()
                            safeDateString = rawDate;
                        }
                    }

                    const dt = new Date(safeDateString || Date.now());
                    const key = formatLocalDateString(dt);
                ...
                const newHistoryMap: Record<string, DayData> = {};
    ...
    let selectedCell = $derived(calendarData.cells.find((cell) => cell.day === selectedDay && !cell.other));
</script>
                ...
                {#each calendarData.cells as cell}
                    ...
                    >
                        ...
                        {#if !cell.other && cell.data}
                            <div class="day-bars">
                                <div class="day-bar" style="height:{Math.min(24, Math.round((cell.data.suhu / 40) * 24))}px;background:#f59e0b"></div>
                                <div class="day-bar" style="height:{Math.min(24, Math.round((cell.data.hum / 100) * 24))}px;background:#38bdf8"></div>
                                <div class="day-bar" style="height:{Math.min(24, Math.round((cell.data.co2 / 1000) * 24))}px;background:#22d6a0"></div>
                            </div>
                        {/if}
                    </button>
                {/each}
